...
predictkde <- function(task = c("learning", "testing", "insample_info"), 
                       selected_bandwiths = NULL, selected_lambdas = NULL){
  ...
  if(task == "learning"){
    ...
    if(grepl("KD-D", algo)){
      ...
      # Should be 5 for weekdays and 3 for weekends. So (3+5)/2 = 4.
      n_approx <- (n_base/336)*4  
    }else{
      stop("error in algo")
    }
    ...
    res <- sapply(seq(10), function(l){
      sapply(seq(48), function(h){bw.nrd0(sample(demand[ids_past][seq(h, length(ids_past), by = 48)], n_approx))})
    })
    ...
    bandwiths_vec <- seq(0, bw_normal, length.out = 5)
    bandwiths_subvec1 <- seq(bandwiths_vec[1], bandwiths_vec[2], length.out = 10)
    bandwiths_subvec2 <- seq(bandwiths_vec[2], bandwiths_vec[3], length.out = 10)
    id <- seq(1, 10, by = 2)[-1]
    bandwiths <- c(bandwiths_subvec1[id], bandwiths_subvec2[id], bandwiths_vec[-seq(3)])
    
    stopifnot(all(bandwiths>0))
    
    nb_futuredays <- length(seq_validation_interval)/48
    lambdas <- c(0.4, 0.6, 0.8, 0.90, 0.95, 1)
...
# KDE for time id_query using data at times ids_data
