...
for(inode in seq_along(agg_nodes)){
  ...
  print(children_nodes)
    ...
    mat_residuals <- sapply(seq_along(children_nodes), function(inode){
      ...
      if(isbottom){
        resid_file <- file.path(in_sample.folder, algo.bottom, paste("residuals_", idseries, "_", algo.bottom, ".Rdata", sep = "")) 
        load(resid_file)
        e_residuals <- c(rep(NA, n_past_obs_kd), e_residuals)
      }else{
        resid_file <- file.path(in_sample.folder, algo.agg, paste("residuals_", idseries, "_", algo.agg, "_", 1, ".Rdata", sep = "")) 
        load(resid_file)
        #e_residuals
      }
      e_residuals
