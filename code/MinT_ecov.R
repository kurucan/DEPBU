...
for(do.agg in c(TRUE, FALSE)){
  ...
  mat_residuals <- sapply(seq_along(set_series), function(j){
    ...
    if(algo == "KD-IC-NML"){
      ...
      resid_MINT_file <- file.path(in_sample.folder, algo, paste("residuals_MINT_", idseries, "_", algo, "_", 1, ".Rdata", sep = "")) 
...
dir.create(file.path(output.folder, "wmatrices"), showWarnings = FALSE, recursive = TRUE)

covmethods  <- c("shrink", "blockdiagonalshrink", "diagonal", "sample")
for(covmethod in covmethods){
  W1file <- file.path(output.folder, "wmatrices", paste("W1_", algo.agg, "_", algo.bottom, "_", covmethod, ".Rdata", sep = "")) 
  if(covmethod == "diagonal"){
    ...
    load(file.path(output.folder, "wmatrices", paste("W1_", algo.agg, "_", algo.bottom, "_", "shrink", ".Rdata", sep = "")))
    W1_bdiagonal <- W1
...
########
  ...
  for(h in seq(48)){
    ...
    # shrink
    Whfile <- file.path(output.folder, "wmatrices", paste("W_", h, "_", algo.agg, "_", algo.bottom, "_", "shrink", ".Rdata", sep = ""))
    ...
    # diagonal
    Whfile <- file.path(output.folder, "wmatrices", paste("W_", h, "_", algo.agg, "_", algo.bottom, "_", "diagonal", ".Rdata", sep = ""))
