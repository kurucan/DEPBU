...
for(idtest in allidtest){
  ...
  res_by_halfhour_file <- file.path(by_halfhour.folder, paste("results_by_halfhour_", algo.agg, "_", algo.bottom, "_", idtest, ".Rdata", sep = "")) 
  load(res_by_halfhour_file)
  ...
  if(do.mint){
    ...
    Whfile <- file.path(output.folder, "wmatrices", paste("W_", hwanted, "_", algo.agg, "_", algo.bottom, "_", "shrink", ".Rdata", sep = ""))
    load(Whfile)
    W1shrink <- W1
    
    Whfile <- file.path(output.folder, "wmatrices", paste("W_", hwanted, "_", algo.agg, "_", algo.bottom, "_", "diagonal", ".Rdata", sep = ""))
    load(Whfile)
    W1diag <- W1
...
res_job <- file.path(error_measures.folder, paste("results_HTS_", algo.agg, "_", algo.bottom, "_", idjob, ".Rdata", sep = "")) 
...
#if(idjob %in% c(1)){
#  samples_job <- file.path(output.folder, "samples_agg", paste("samples_agg_", algo.agg, "_", algo.bottom, "_", idjob, ".Rdata", sep = "")) 
#  save(file = samples_job, list = c("list_samples_agg"))
#}
