...
load(file.path(output.folder, "info.Rdata"))

# Regions available: "UKG" "UKL" "UKJ" "UKI" "UKM" "UKF" "---" "UKK" "UKD"
myregion <- "UKF" # We focus on the UKF region
...
for(i in seq_along(metersInInterval)){
  ...
  load(file.path(init_meters.folder, paste("meter-", meter, ".Rdata", sep = "")))
  n[i] <- nrow(dataset)
...
id <- which(pctFound > 0.99) # What are the meters with less than 1% missing values? 
...
# since the length of idset is not even, I remove the first one and add it later
idset_1 <- idset[1]
idset <- idset[-1]
res <- split(idset, c(1,2))

x[res[[1]], "NUTS4"] <- "UKF2100"
x[c(idset_1, res[[2]]), "NUTS4"] <- "UKF2101"
...
save(file = file.path(output.folder, "myinfo.Rdata") , 
     list = c("myinfoDT", "bottomSeries", "itree", "Sagg", "aggSeries", "n_agg", "n_bottom"))
