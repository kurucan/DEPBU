...
# 14319 meters (not 14621 meters = 16249 - 1628)
...
save(file = file.path(output.folder, "info.Rdata") , list = c("infoDT", "allmeters"))

# Create a file for each meter (Takes time !)
if(TRUE){
  print("Making files for each meter")
  setmeters <- allmeters

  res <- sapply(setmeters, function(idmeter){
    ...
    save(file = file.path(init_meters.folder, paste("meter-", idmeter, ".Rdata", sep = "")) , list = c("dataset"))
